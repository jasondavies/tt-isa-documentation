...
> [!TIP]
> PCBufs in Blackhole are the same as those in Wormhole.
...
Note that the `OverrideEn` and `OverrideBusy` configuration can also affect [Manual TTSync](ManualTTSync.md).
